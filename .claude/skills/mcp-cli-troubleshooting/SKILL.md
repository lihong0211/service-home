...
description: "Diagnosing and resolving MCP server and CLI tool integration issues with Codex — permission bottlenecks, tool selection, diagnostic interpretation, next-step guidance. Use this skill when installing MCP servers or CLI tools in Codex, troubleshooting integration issues (permission errors, diagnostic warnings, missing configuration), or interpreting health-check output."
...
updated_at: 2026-07-14T22:27:16.843Z
---
...
- **Distinguish issues:**
  - Permission/sandbox bottleneck: "awaiting approval," system dialog hanging, permission denied on `~/.config`
...
- **Recognize patterns:** If permission prompt appeared, user may need to click *button* in GUI context, not type in chat. If in headless/CLI context, offer terminal workaround to bypass Codex sandbox.

### 2. Tool Comparison (when user is choosing between alternatives)
...
- **Example:** `codebase-memory-mcp` vs `Headroom`
  - `codebase-memory-mcp`: knowledge graph → helps AI understand code structure (reduces exploration)
  - `Headroom`: output compression → shrinks logs/RAG/tool responses (reduces transmission)
...
- **Parse output:** Extract pass/warn/fail checks. Identify which are critical vs. informational.
  - *Critical:* Core feature doesn't work (e.g., proxy not running → Headroom can't compress tokens)
  - *Warning:* Nice-to-have not configured (e.g., budget limits, shell env fallback, persistence setup)
  - *Failure:* Non-critical subsystem down (e.g., persistent deployment offline, but manual proxy available)
- **Key insight:** Many warnings don't block primary workflow. Example: `deployments ✗ init-user down` is OK if `proxy ✓ pass` and `codex ✓ routed` — you can use Headroom now, just not auto-start on reboot yet.
...
- Parse results against readiness criteria (e.g., "When you see ✓ on proxy and ✓ on codex, you're ready to use; deployments fail is OK for now")
...
- User in *interactive Codex GUI:* "Click the permission dialog button that appears"
...
When tool outputs a table like `headroom doctor`:
```
...
- **Separate tiers:** Critical (must pass for tool to function) vs. Optional (nice-to-have)
- **Explain each warn/fail:** What does it mean in practice? Does it block your workflow?
  - Example: `claude ⚠ not routed` doesn't affect Codex; it only affects Claude Code CLI
  - Example: `deployments ✗ fail` means Headroom won't auto-start on reboot, but manual proxy still works
  - Example: `shell env ⚠ unset` means direct terminal requests bypass proxy, but pre-configured Codex clients use it
...
4. **Verify:** After restart, `savings` shows token count. You confirm: "Headroom is now active in Codex."
