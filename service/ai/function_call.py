...
async def function_calling_info_api(request: Request):
    try:
        return {"code": 0, "msg": "ok", "data": get_function_calling_info()}
    except Exception as e:
        return ({"code": 500, "msg": str(e), "data": None}, 500)


async def function_calling_chat_api(request: Request):
    body = await read_json_optional(request) or {}
    messages = body.get("messages", [])
    if not messages:
        return ({"code": 400, "msg": "请提供 messages"}, 400)
    try:
        ...
        if isinstance(out, dict) and out.get("error"):
            return ({"code": 500, "msg": out["error"]}, 500)
