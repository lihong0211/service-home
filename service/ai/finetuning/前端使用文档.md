...
// 类型示意（前端可自建 interface）
{
  ...
  lora?: "medical" | "legal";   // 选用的 LoRA：medical=医疗问诊，legal=法律咨询。不传默认为 medical
  model_type?: "medical" | "legal";  // 同上，与 lora 二选一即可
  options?: {        // 生成参数，可选
...
- **lora** / **model_type**：**重要**。选择「医疗问诊 LoRA」时传 `"medical"`，选择「法律咨询 LoRA」时传 `"legal"`。不传则后端默认使用医疗 LoRA；若界面选了法律但未传 `lora: "legal"`，右侧会误用医疗 LoRA 的回复。
