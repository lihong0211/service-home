...
# coding: utf-8
"""
Qwen2.5-7B 医疗微调 - Hugging Face 原生版（无 Unsloth）
可在 Mac（MPS/CPU）、Linux（CUDA）上运行；Mac 上速度较慢，建议小 batch、少 epoch 先跑通。
"""
...
if not os.path.isdir(BASE_MODEL_PATH):
    BASE_MODEL_PATH = os.path.join(_SCRIPT_DIR, "models", "Qwen", "Qwen2.5-7B-Instruct")
_LOCAL_MODEL = os.path.isdir(BASE_MODEL_PATH)  # 本地目录则 from_pretrained 用 local_files_only
...
_RUN_PARENT = get_run_parent_dir(
    get_finetuning_root(), model_name="Qwen2.5-7B-Instruct"
)
...
if _LOCAL_MODEL:
    tokenizer = PreTrainedTokenizerFast(tokenizer_file=os.path.join(BASE_MODEL_PATH, "tokenizer.json"))
...
# MAX_TRAIN_SAMPLES: 本地 Mac 调试用，设为 None 则使用全量数据
MAX_TRAIN_SAMPLES = 200

dataset = load_medical_data(MEDICAL_DATA_DIR)
if MAX_TRAIN_SAMPLES is not None:
    dataset = dataset.select(range(min(MAX_TRAIN_SAMPLES, len(dataset))))
    print(f"截取前 {len(dataset)} 条用于本地测试")
dataset = dataset.map(formatting_prompts_func, batched=True)

# ---------- 训练参数（Mac 上建议小 batch）----------
# 新版 TRL 使用 SFTConfig 代替 TrainingArguments，dataset_text_field / max_length / packing 在此传入
# 全量训练：加大 accumulation 减少步数，减少 epoch；7B 在 MPS 上保持 batch=1 省显存
_use_full_data = MAX_TRAIN_SAMPLES is None
batch_size = 1 if device in ("mps", "cpu") else 4
